...
export const exportMessagesToExcel = async () => {
  ...
  let tableOrderNumber = 1; // Порядковый номер таблицы, начиная с 1
  ...
  allTables.forEach((table) => {
    ...
    table.rows.forEach((row) => {
      const rowData: Record<string, string | number> = {
        "№ п.п": tableOrderNumber, // Используем порядковый номер таблицы
      };
    ...
    tableOrderNumber++; // Увеличиваем порядковый номер для следующей таблицы
  });
