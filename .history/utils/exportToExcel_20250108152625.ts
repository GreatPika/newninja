...
export const exportMessagesToExcel = async () => {
  ...
  const mergedRanges = worksheet._merges; // Получаем список объединенных ячеек
  ...
  for (let row = 2; row < currentRowNumber; row++) {
    ...
    // Условие 3: Если ячейка C объединена
      ...
      if (mergedRange) {
        ...
        // Заполняем весь объединенный диапазон
        worksheet.mergeCells(`F${startRow}:${mergedRange.split(':')[1]}`);
