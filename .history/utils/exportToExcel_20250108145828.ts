...
export const exportMessagesToExcel = async () => {
  ...
  worksheet.columns = [
    ...
    { header: "Инструкция", key: "instruction", width: 50 }, // Добавляем колонку "Инструкция"
  ];
  ...
  // Логика объединения ячеек в третьей колонке (C) и заполнения шестой колонки (F)
  let mergeStart = 2;
  let lastValue = "";
  ...
  for (let row = 2; row <= currentRowNumber; row++) {
    ...
    if (cellValue) {
      if (row - 1 >= mergeStart) {
        ...
        // Заполнение колонки F для объединённых ячеек
    ...
    if (row === currentRowNumber && lastValue && row > mergeStart) {
      // Объединение оставшихся ячеек в колонке C
  ...
  // Дополнительная логика для строк, где колонка C и D имеют значения
  worksheet.eachRow((row, rowNumber) => {
    if (rowNumber < 2) return; // Пропускаем заголовок
    const colC = worksheet.getCell(`C${rowNumber}`).value as string;
    const colD = worksheet.getCell(`D${rowNumber}`).value as string;
