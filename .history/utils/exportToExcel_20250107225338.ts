...
export const exportMessagesToExcel = async () => {
  ...
  allTables.forEach((table, tableIndex) => {
    let rowIndex = 1; // Добавляем счетчик для нумерации строк внутри таблицы

    table.rows.forEach((row) => {
      const rowData: Record<string, string | number> = {
        "№ п.п": `${tableIndex + 1}.${rowIndex}`, // Формируем номер в формате "1.1", "1.2" и т.д.
      };
      ...
      currentRowNumber++;
      rowIndex++; // Увеличиваем счетчик для следующей строки
    });
  });
