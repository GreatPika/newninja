  ...
  resolvedTables.forEach((table, tableIndex) => {
    ...
    table.rows.forEach((row) => {
      const instruction = (() => {
        ...
        const hasComparisonSigns = /[≥≤<>]/;

        if (!hasComparisonSigns.test(colC) && !hasComparisonSigns.test(colD)) {
          return "Значение характеристики не может изменяться участником закупки";
        }

        if (hasComparisonSigns.test(colD)) {
    ...
    if (startRow < currentRowNumber) {
      ...
      worksheet.mergeCells(
        `F${startRow}:F${currentRowNumber - 1}`,
      );
      const mergedInstructionCell = worksheet.getCell(`F${startRow}`);
      mergedInstructionCell.value =
        "Участник закупки указывает в заявке все значения характеристики";
      mergedInstructionCell.alignment = defaultAlignment;
    }
  });
