...
import { saveMessageToDB } from "@/utils/messages";
        ...
        if (line.startsWith("data:")) {
          try {
            ...
            if (data.type === "status") {
              ...
              // Сохраняем пронумерованные предложения в IndexedDB
              await saveMessageToDB({
                text: JSON.stringify(data.data),
                role: "system",
                timestamp: new Date(),
                source: "numbered_sentences",
              });
    ...
    if (buffer?.startsWith("data:")) {
      try {
        ...
        if (data.type === "status") {
          ...
          enqueueSnackbar(`Ошибка: ${data.message}`, { variant: "error" });
          return null;
        } else if (data.type === "raw_json") {
          // Сохраняем сырой JSON в IndexedDB
          await saveMessageToDB({
            text: JSON.stringify(data.data),
            role: "system",
            timestamp: new Date(),
