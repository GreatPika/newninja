...
export const exportMessagesToExcel = async () => {
  ...
  // 1. Анализ значений C и D для всех строк
  for (let row = 2; row < currentRowNumber; row++) {
    ...
    // Пропускаем строки, которые входят в объединенные диапазоны
    ...
    if (valueC && valueD) {
      ...
      // Пустое значение в C или D
      cellF.value = "Некорректные данные: отсутствует значение в колонке C или D";
    }
  }

  // 2. Объединение ячеек в колонке C и запись данных о диапазонах
  let mergeStart = 2;
  let lastValue = "";
  ...
  // 3. Обработка объединенных диапазонов в колонке F
