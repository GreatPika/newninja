...
export const exportMessagesToExcel = async () => {
  ...
  let mergeStart = 2;
  let lastValueC = "";
  let isMerging = false; // Флаг для отслеживания процесса объединения
  ...
  for (let row = 2; row <= currentRowNumber; row++) {
    ...
    // Если не в процессе объединения, используем стандартную логику getInstruction
    if (!isMerging) {
      const instruction = getInstruction(cellValueC, cellValueD);
      worksheet.getCell(`F${row}`).value = instruction;
    }
    ...
    if (cellValueC) {
      ...
      isMerging = false; // Сбрасываем флаг после завершения объединения
    } else {
      isMerging = true; // Устанавливаем флаг, когда находимся в процессе объединения
    }
