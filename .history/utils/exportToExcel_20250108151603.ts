...
export const exportMessagesToExcel = async () => {
  ...
  for (let row = 2; row <= currentRowNumber; row++) {
    ...
    // Проверка на изменения значений в колонках C и D
    if (lastValue !== colCValue || row === currentRowNumber) {
      if (row - 1 >= mergeStart) {
    ...
    // Если значения C и D не содержат специальных символов
    if (
      colCValue &&
      colDValue &&
      !/[≥≤><]/.test(colCValue) &&
    ...
    // Если в значениях D есть специальные символы
    if (/[≥≤><]/.test(colDValue)) {
      worksheet.getCell(`F${row}`).value =
