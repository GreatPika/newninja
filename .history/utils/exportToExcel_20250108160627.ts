...
export const exportMessagesToExcel = async () => {
  ...
  resolvedTables.forEach((table, tableIndex) => {
    ...
    table.rows.forEach((row) => {
      worksheet.addRow({
        ...
        instruction: "", // Временно оставляем пустым, заполним позже
      });
      currentRowNumber++;
    });
  ...
  // Обработка объединения ячеек и установка инструкций
  for (let row = 2; row <= currentRowNumber; row++) {
    const cellValueC = worksheet.getCell(`C${row}`).value;
    const cellValueD = worksheet.getCell(`D${row}`).value;

    if (cellValueC) {
      // Обработка предыдущего блока объединенных ячеек
      if (row - 1 >= mergeStart) {
        ...
        if (!mergeCellC.isMerged) {
          ...
          // Устанавливаем специальное значение инструкции для объединенных ячеек
      ...
      // Устанавливаем инструкцию для одиночной строки
      worksheet.getCell(`F${row}`).value = getInstruction(cellValueC as string, cellValueD as string);
    } else {
      // Для строк внутри объединенного блока инструкция пока не устанавливается
    ...
    if (row === currentRowNumber && lastValueC && row > mergeStart) {
      ...
      if (!mergeCellC.isMerged) {
        worksheet.mergeCells(`C${mergeStart}:C${row - 1}`);
        mergeCellC.value = lastValueC;
        // Устанавливаем специальное значение инструкции для последнего блока объединенных ячеек
        worksheet.mergeCells(`F${mergeStart}:F${row - 1}`);
        mergeCellF.value = "Участник закупки указывает в заявке все значения характеристики";
      }
    }
...
const getInstruction = (colC: string, colD: string) => {
  const containsComparisonOperators = (value: string) =>
