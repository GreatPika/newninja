...
export const exportMessagesToExcel = async () => {
  ...
  const mergeInfo: { startRow: number; endRow: number }[] = [];
  ...
  for (let row = 2; row <= currentRowNumber; row++) {
    ...
    if (cellValue) {
      if (row - 1 >= mergeStart) {
        ...
        mergeInfo.push({ startRow: mergeStart, endRow: row - 1 });
      }
      mergeStart = row;
    ...
    if (row === currentRowNumber && lastValue && row > mergeStart) {
      ...
      mergeInfo.push({ startRow: mergeStart, endRow: row });
    }
  }

  worksheet.spliceRows(currentRowNumber, 1);

  // Заполнение колонки F
  for (let row = 2; row <= currentRowNumber; row++) {
    ...
    // Проверяем, входит ли строка в объединенный диапазон
    const isMerged = mergeInfo.some(
      ({ startRow, endRow }) => row >= startRow && row <= endRow,
    );

    if (isMerged) {
      // Если строка объединена, ничего не делаем — объединенные ячейки обрабатываются отдельно
      continue;
    }

    // Обработка не объединенных строк
    if (valueC && valueD) {
      if (!/[≥≤><]/.test(valueC)) {
        cellF.value =
      ...
      cellF.value = "Некорректные данные: отсутствует значение в колонке C или D";
    }
  }

  // Обработка объединенных диапазонов в колонке F
