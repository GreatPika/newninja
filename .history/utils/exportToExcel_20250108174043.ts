...
export const exportMessagesToExcel = async () => {
  ...
  // Логика добавления "Инструкции" в колонку F
  ...
  for (let i = 2; i <= lastRow; i++) {
    ...
    if (cellC.isMerged) {
      ...
      const mergeEndRow = mergeArea.row + (mergeArea.rowSpan || 1) - 1;

      // Если это первая строка объединенного диапазона
      if (mergeArea.row === i) {
        // Объединяем соответствующие ячейки в колонке F
        worksheet.mergeCells(`F${mergeArea.row}:F${mergeEndRow}`);
        worksheet.getCell(`F${mergeArea.row}`).value =
