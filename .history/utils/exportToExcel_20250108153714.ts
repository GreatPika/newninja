...
export const exportMessagesToExcel = async () => {
  ...
  for (let row = 2; row < currentRowNumber; row++) {
    ...
    // Условие 3: Если строка объединена
    else if (isMerged) {
      ...
      if (mergedRange) {
        ...
        // Заполняем весь объединенный диапазон в колонке F
        if (row === startRow) {
          for (let mergedRow = startRow; mergedRow <= Number(mergedRange.split(':')[1].replace(/\D/g, "")); mergedRow++) {
            worksheet.getCell(`F${mergedRow}`).value =
