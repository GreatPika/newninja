...
import { Message, ChatContainerProps, RunFlowParams, LangflowResponse } from '@/lib/types';
...
export function ChatContainer({ flowId, apiKey, baseURL }: ChatContainerProps) {
  ...
  const handleSubmit = useCallback(async (text: string) => {
    ...
    try {
      ...
      const responseMessage = response?.outputs?.[0]?.outputs?.[0]?.outputs?.message?.message?.text;
