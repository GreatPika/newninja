...
export default function EditPage() {
  ...
  const convertSourceToText = (source: string | object, rowNumber?: number) => {
    try {
      const sourceObj = typeof source === "string" ? JSON.parse(source) : source;
      
      if (rowNumber !== undefined) {
        return sourceObj[rowNumber] || "";
      }
  ...
  const handleTableClick = async (rowNumber: number) => {
    setActiveTableRow(rowNumber);
    
    if (messageId) {
      try {
        const message = await getMessageById(messageId);
        if (message?.source) {
          const sourceContent = convertSourceToText(message.source, rowNumber);
    ...
    <div style={{ height: "100vh", overflow: "hidden" }}>
      <PanelGroup direction="vertical">
        ...
        >
          <div style={{ height: "100%" }}>
            <Editor
              ...
              onTableRowClick={handleTableClick}
            />
          </div>
        </Panel>
        ...
        >
          <div style={{ height: '100%' }}>
            <Editor
              markdown={activeTableRow ? sourceContent : "Выберите строку в таблице"}
              showToolbar={false}
              onContentChange={() => {}}
              readOnly={true}
            />
