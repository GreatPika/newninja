...
export default function EditPage() {
  ...
  const [activeSourceValue, setActiveSourceValue] = useState('');
  ...
  const findSourceLine = (value: string) => {
    if (!value || !sourceContent) return '';
    ...
    <div style={{ height: "100vh", overflow: "hidden" }}>
      <PanelGroup direction="vertical">
        ...
        >
          <div style={{ height: "100%" }}>
            <Editor
              key={markdown}
              markdown={markdown}
              onContentChange={(value) => setActiveSourceValue(value)}
            />
          </div>
        </Panel>
        ...
        >
          <div style={{ height: "100%" }}>
            <Editor
              markdown={findSourceLine(activeSourceValue) || 'Не найдено соответствующего источника'}
              showToolbar={false}
              onContentChange={() => {}}
            />
          </div>
