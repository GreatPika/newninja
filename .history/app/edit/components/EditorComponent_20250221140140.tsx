    ...
    const handleClick = (e: Event) => {
      ...
      if (cell) {
        ...
        if (row && table) {
          ...
          const column5Content = cells[4]?.textContent?.trim() || 'н/д'; // Добавили trim() для очистки пробелов
          
          console.log(`Строка: ${rowIndex}, Колонка 5: ${column5Content}`);
          setActiveRow(rowIndex);
          setColumn4Value(column5Content); // Можно переименовать состояние при необходимости
          return;
        }
      }
  ...
  return (
    <div>
      ...
      {activeRow !== null && (
        ...
        >
          Активная строка: {activeRow}, Значение в 5 колонке: {column4Value ?? 'н/д'}
        </div>
      )}
    </div>
  );
