...
const Editor: FC<EditorProps> = ({
  ...
  useEffect(() => {
    const handleTableClick = () => {
      const editor = rootEditor?.value as LexicalEditor | null;
      
      editor?.getEditorState()?.read(() => {
        const nodes = currentSelection?.value?.nodes;
        if (!nodes || nodes.length < 2) return;

        const [tableNode, cellNode] = nodes;
        if (tableNode?.__type === 'table') {
          const [rowIndex] = (tableNode as TableNode).getCoordsFromCellNode(cellNode);
