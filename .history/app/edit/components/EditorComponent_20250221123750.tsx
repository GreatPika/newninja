...
interface EditorProps {
  ...
  showToolbar?: boolean;
  onActiveRowChange?: (data: { row: number; value: number } | null) => void;
}
    ...
    const handleClick = (e: Event) => {
      ...
      if (cell) {
        ...
        if (row && table) {
          ...
          const fourthCellContent = cells[3]?.textContent?.trim() || '';
          const cellValue = parseInt(fourthCellContent);

          setActiveRow(rowIndex);
          onContentChange?.(cellValue.toString());
          return;
        }
      }
