...
const Editor: FC<EditorProps> = ({
  ...
  const [currentSelection, rootEditor] = useCellValues<[typeof currentSelection$, typeof rootEditor$]>(
  ...
  useEffect(() => {
    const handleTableClick = () => {
      (rootEditor as LexicalEditor | null)?.getEditorState()?.read(() => {
        const nodes = (currentSelection as any)?.nodes;
        if (!nodes) return;

        const node = nodes[0];
        if (node?.__type === "table") {
          const tableNode = node as TableNode;
          const [rowIndex] = tableNode.getCoordsFromCellNode(nodes[1]);
