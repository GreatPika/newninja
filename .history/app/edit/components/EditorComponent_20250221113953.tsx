...
const Editor: FC<EditorProps> = ({
  ...
  useEffect(() => {
    const handleTableClick = () => {
      (rootEditor as LexicalEditor)?.getEditorState()?.read(() => {
        const node = (currentSelection as any)?.nodes?.[0];

        if (node && node.__type === "table") {
          const tableNode = node as TableNode;
          const [rowIndex] = tableNode.getCoordsFromCellNode(
            (currentSelection as any)?.nodes?.[1]
          );
